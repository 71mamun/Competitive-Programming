...
{
    int tot_prime = seive();
    cumulative[0] = primes[0];
    For(i,1,tot_prime-1)
    {
        cumulative[i] = (ui)primes[i]*cumulative[i-1];
    }
    int T,n,t=1;
    tk(T);
    while(T--)
    {
        ...
        int idx = upper_bound(primes,primes+tot_prime,n)-primes;
        pf("Case %d: %u\n",t++,ans*cumulative[idx-1]);
    }
    return 0;
}
