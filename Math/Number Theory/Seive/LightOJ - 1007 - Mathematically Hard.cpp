...
/**
    ...
    at first we keep n into every number phi value then prime by prime
    add thats prime contribution into the phi value of a number.
*/

int seive()
{
    int i,j,total=0,val;
    for(i=2;i<LM;++i)flag[i]=1;
    val = sqrt(LM)+1;
    for(i=2;i<val;++i)
    {
        if(flag[i])
...
{
    int tot_prime = seive();
    for(int i=1;i<LM;++i)phi[i]=i;
    phi[1]=0;
    for(int i=0;i<tot_prime;++i)
    {
        int p = primes[i];
        for(int j=p;j<LM;j+=p)
        {
            phi[j] *= (p-1);
            phi[j] /= p;
        }
