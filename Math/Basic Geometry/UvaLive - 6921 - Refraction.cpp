...
///~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//struct tpl{double f,s,t;tpl(){}tpl(double a,double b,double c){f=a,s=b,t=c;}};
//int cross_product(tpl a,tpl b){int v=a.f*(b.s-b.t)-a.s*(b.f-b.t)+a.t*(b.f-b.s);return (v==0?0:(v>0?+1:-1));}
//tpl make_vector(tpl s,tpl e){tpl a;a.f=e.f-s.f;a.s=e.s-s.s;a.t=e.t-s.t;return a;}
///~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
tpl create_equation(pdd p, pdd q)
{
    double a = (p.sc-q.sc);
    double b = -(p.fs-q.fs);
...
pdd calculate_intersection(tpl e1, tpl e2)
{
...
double cal(double hig)
{
    tpl eequ = create_equation(ey,be);
    tpl hequ = create_equation(mp(0,hig),mp(W,hig));
    sec = calculate_intersection(eequ,hequ);
    double otivug1 = pdis(sec,mp(XE,YE));
    double lombo1 = XE-sec.fs;
    double otivug2 = pdis(mp(X,0),sec);
    double lombo2 = sec.fs-X;
    double dun = (otivug1*lombo2);
    double num = (lombo1*otivug2);
    return num/dun<=u and sec.fs>X;
}
...
//    fread("in.txt");
    ...
    while(T--)
    {
        ...
        be = mp(W,H);
        ey = mp(XE,YE);bool imp=false;
        double hi = H,lo = 0,mid;
        if(!cal(H))imp=true;
        while(true)
        {
            mid = (hi+lo)/2;
            if(abs(hi-lo)<eps)break;
            if(cal(mid))
            {
                hi = mid;
            }
