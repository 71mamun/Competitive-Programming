...
triple add(triple a,triple b,double fev,double tot)
{
    double f = fev/tot;
    return triple(f*b.f+a.f,f*b.s+a.s,f*b.t+a.t);
}


triple sol(int r,int s,int p)
{
    if(r<0||s<0||p<0)return triple(0,0,0);
    if(!r||!s||!p)return triple(!p,!r,!s);

    triple &ans = dp[r][s][p];
    if(vis[r][s][p])return ans;
    vis[r][s][p] = 1;
    ans = triple(0,0,0);

    double tot = r*s+s*p+r*p;
    ans = add(ans,sol(r,s-1,p),r*s,tot);
    ans = add(ans,sol(r,s,p-1),s*p,tot);
    ans = add(ans,sol(r-1,s,p),r*p,tot);
...
int main()
{
    int r,p,s;
    sf("%d %d %d",&r,&s,&p);
    triple ans = sol(r,s,p);
    pf("%.12lf %.12lf %.12lf",ans.f,ans.s,ans.t);
    return 0;
}
