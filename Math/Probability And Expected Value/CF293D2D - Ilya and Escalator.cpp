...
double dp[LM][LM];
bool ted[LM][LM];
double solve(int time_baki,int man_baki)
{
    if(time_baki==0 or man_baki ==0)return n-man_baki;// man_ase = tot_man - man_baki.
    double &ret = dp[time_baki][man_baki];
    if(ted[time_baki][man_baki])return ret;
    ted[time_baki][man_baki]=true;
    ret+=solve(time_baki-1,man_baki-1)*p;
    ret+=solve(time_baki-1,man_baki)*(1-p);
    return ret;
}
