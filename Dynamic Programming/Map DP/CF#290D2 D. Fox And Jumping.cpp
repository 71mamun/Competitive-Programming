...
int sol(int pos,int gcd)
{
    if(pos>=n||gcd==1)return (gcd==1?0:inf);
    if(dp[pos][gcd])return dp[pos][gcd];
    return dp[pos][gcd] = min(sol(pos+1,gcd),sol(pos+1,__gcd(gcd,l[pos]))+c[pos]);
}
...
int main()
{
    while(cin>>n)
    {
        ...
        cout<<(sol(0,0)<inf?sol(0,0):-1)<<endl;
    }
    return 0;
}
