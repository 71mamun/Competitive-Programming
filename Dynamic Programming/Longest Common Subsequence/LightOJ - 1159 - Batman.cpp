...
#define LIM (55)
char a[LIM],b[LIM],c[LIM];
int lcs[LIM][LIM][LIM];

int lcs3D(int i,int j,int k)
{
    int &ret = lcs[i][j][k];
    if(~ret)return ret;
    if(i<=0 or j<=0 or k<=0)return ret = 0;
    if(a[i-1]==b[j-1] and b[j-1]==c[k-1])
    {
        return ret = lcs3D(i-1,j-1,k-1)+1;
    }
    ret = -inf;
    ret = max(ret,lcs3D(i-1,j,k));
    ret = max(ret,lcs3D(i,j-1,k));
    ret = max(ret,lcs3D(i,j,k-1));
    ret = max(ret,lcs3D(i-1,j-1,k));
    ret = max(ret,lcs3D(i,j-1,k-1));
    ret = max(ret,lcs3D(i,j-1,k-1));
    return ret;
}
