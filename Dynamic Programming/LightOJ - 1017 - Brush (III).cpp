...
int solve(int crnt,int tkn)
{
    if(tkn>=k or crnt>=n)return 0;
    int &ret = DP[crnt][tkn];
    if(~ret)return ret;
    int ltkn=1;
    int next = crnt+1;
    while(pnt[next].sc-pnt[crnt].sc<=w and next<n and crnt<n)
    {
        ltkn++;
        next++;
    }
    ret = solve(crnt+1,tkn);
    ret = max(ret,solve(next,tkn+1)+ltkn);
    return ret;
}
