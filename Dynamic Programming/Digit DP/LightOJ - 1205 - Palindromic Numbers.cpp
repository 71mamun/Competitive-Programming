...
ll sol(ll n)
{
    ...
    if(n<0)return 0;
    if(~dp[n])return dp[n];
    return dp[n] = 9*pwr(10LL,(ll)ceil(double(n)/2)-1)+sol(n-1);
}
...
ll cal(ll n)
{
    ...
    ll str_n_len  = str_n.size();
    ll hlf = (ll)ceil(double(str_n_len )/2);

    ll ans = sol(str_n_len-1);

    ll bgn = pwr(10LL,hlf-1);
    ll edn = to_int(str_n.substr(0,hlf));


    string fsts = str_n.substr(0,(str_n_len %2==0?hlf:hlf-1));
    string lsts = str_n.substr(hlf,(str_n_len %2==0?hlf:hlf-1));
    reverse(all(fsts));
    ll fst = to_int(fsts);
    ll lst = to_int(lsts);

    ans+=(edn-bgn)+(fst<=lst?1:0);
