...
ll dp[2][10][105][2];
bool flag[2][10][105][2];

ll solve(bool les, int pre, int pos,int ok,string &a)
{
    if(pos==a.size())return ok;
    ll &ans = dp[les][pre][pos][ok];
    if(flag[les][pre][pos][ok]==true)return ans;
    flag[les][pre][pos][ok] = true;
    ans = 0;ll tmp;
    ll mx = (les?a[pos]-'0':9);
    For(i,0,mx)
    {
        ans += solve(les&&i==mx,i,pos+1,ok||(pre==4 && i == 9),a);
    }
    return ans;
}
