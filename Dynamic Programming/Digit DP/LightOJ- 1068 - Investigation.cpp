...
ll T,a,b,k,t=1,ans,lo,hi;
ll shift[] = {1LL,10LL,100LL,1000LL,10000LL,100000LL,1000000LL,10000000LL,100000000LL,1000000000LL,10000000000LL};
ll dp[2][11][90][90];
ll sol(ll les,ll pos,ll sum_of_digit,ll sum_of_value,ll v,ll k)
{
//    put(les,pos,sum_of_digit,sum_of_value,v,k);
    if(pos<0)return sum_of_digit==0 and sum_of_value==0;
    ll &ans = dp[les][pos][sum_of_digit][sum_of_value];
    if(~ans)return ans;
    ans = 0;
    int hi;
    if(les)hi=9;
    else hi = (v/shift[pos])%10;
    For(i,0,hi)
    {
        if(i==hi and !les)ans+=sol(0,pos-1,(sum_of_digit+i)%k,(sum_of_value+i*shift[pos])%k,v,k);
        else ans+=sol(1,pos-1,(sum_of_digit+i)%k,(sum_of_value+i*shift[pos])%k,v,k);
    }
    return ans;
}
...
int main()
{
    ...
    while(T--)
    {
        ...
        mem(dp,-1);
        ll hi = sol(0,9,0,0,b,k);
        mem(dp,-1);
        ll lo = sol(0,9,0,0,a-1,k);
        pf("%lld\n",hi-lo);
    }
    return 0;
}
