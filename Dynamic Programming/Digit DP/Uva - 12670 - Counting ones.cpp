...
pll solve(bool in,ll pos,ll val)
{
    if(pos<0)
    {
        return mp(0,1);
    }
    if(flag[in][pos])return result[in][pos];
    flag[in][pos]=true;
    pll &ans = result[in][pos],tmp;
    ans = mp(0,0);
    if(in)
    {
        tmp = solve(1,pos-1,val);
        ans.fs+=(tmp.fs+tmp.sc);
        ans.sc+=(tmp.sc);
        tmp = solve(1,pos-1,val);
        ans.fs+=(tmp.fs);
        ans.sc+=(tmp.sc);
...
int main()
{
    ...
    {
        mem(flag,false);
        ll hi = solve(0,62,b).fs;
        mem(flag,false);
        ll lo = solve(0,62,a-1).fs;
        put(hi-lo);
    }
    return 0;
}
