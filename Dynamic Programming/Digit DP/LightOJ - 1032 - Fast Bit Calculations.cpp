...
ll solve(bool les, bool pre,int pos)
{
    ll &ans = DP[les][pre][pos];
    if(~ans) return ans;
    ...
    if(les==true)
    {
        if(pre==true)
        {
            ans += solve(true,true,pos-1)+pwr(2,pos);
            ans += solve(true,false,pos-1);
        }
        else
        {
            ans += solve(true,true,pos-1);
            ans += solve(true,false,pos-1);
        }
    }
    else
    {
        if(isOn(n,pos))
        {
            if(pre==true)
            {
                ans+=solve(false,true,pos-1)+cal(pos-1);
                ans+=solve(true,false,pos-1);
            }
            if(pre==false)
            {
                ans+=solve(false,true,pos-1);
                ans+=solve(true,false,pos-1);
            }
        }
        else
        {
            ans+=solve(false,false,pos-1);
        }
    }
    return ans;
}
