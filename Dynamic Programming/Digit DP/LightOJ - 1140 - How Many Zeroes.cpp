...
pll sol(int start,int gtl,int lth,int pos)
{
    if(pos==len)return mp(0,1);
    pll &ans = dp[start][gtl][lth][pos];
    bool &flag = vis[start][gtl][lth][pos];
    if(flag)return ans;
    flag = true;
    int st,en;
    if(gtl)st = 0;
    ...
    for(int i=st;i<=en;++i)
    {
        pll tmp = sol(start|(i!=0)|pos==len-2,gtl|(i!=st),lth|(i!=en),pos+1);
        if(i==0&&start) ans.fs += (tmp.sc+tmp.fs);
        else ans.fs += tmp.fs;
        ans.sc += tmp.sc;
    }
    return ans;
