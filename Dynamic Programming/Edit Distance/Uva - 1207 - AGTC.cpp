...
/// time O(nm), space O(nm),back track path print O(n+m) uva gives CE due to comment out :'(
#define LM 2000
int fl,sl,dp[LM][LM];
string f,s;
...
{
    /**
    ed of a to b = ed of b to a.
    ed among empty string is 0.
    ed among empty string and another
    string is the length of other
    string through insertion one by one.
    shortcut base case:
    ...
    */
    ...
    ans = inf;
    /**
    Case 1: Align right char from 1st string
    and no char from 2nd string.
    We need a deletion (D) here.
    We still left with problem E(i-1, j).
    Case 2: Align right char from 2nd string and
    no char from 1st string.
    We need an insertion (I) here.
    We still left with problem E(i, j-1).
    Case 3: Replace the right char of 1st string with
    right char of 2nd string if they are not equal
    else no edit is required.
    We still left with the problem E(i-1, j-1).
    the table is 1 indexed while the string is 0 indexed
    */
    ans = min(ans,solve(i-1,j)+1);
    ans = min(ans,solve(i,j-1)+1);
    ans = min(ans,solve(i-1,j-1)+(f[i-1]==s[j-1]?0:1));
    return ans;
}
