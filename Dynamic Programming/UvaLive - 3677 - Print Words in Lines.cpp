...
int solve(int line_no,int now_word)
{
    if(now_word>=n)return 0;
    int already_given=wsz[now_word];
    int &ans = dp[line_no][now_word];
    if(~ans)return ans;
    ans = solve(line_no+1,now_word+1)+sqr(m-already_given);
    for(int i = now_word+1;already_given+wsz[i]+1<=m and i<n;++i)
    {
        already_given+=(wsz[i]+1);
        ans = min(ans,solve(line_no+1,i+1)+sqr(m-already_given));
    }
    return ans;
}
