...
ll kmp_dp(ll len,ll now)
{
    ll &ret = kmpdp[len][now];
    if(~ret)return ret;
    while(len>0 && tword[len]!=now)
        len = lps[len-1];
    if(tword[len]==now)len++;
    return ret = len;
}
...
pii sol(ll len, ll pos)
{
    ...
    for(ll i=0;i<K;++i)
    {
        ll nlen = kmp_dp(len,keys[i]);
        pii tmp = sol(nlen,pos+1);
        ans.sc = max(ans.sc,tmp.sc+(nlen>=sz(tword)));
        ans.fs += ((tmp.fs+(nlen>=sz(tword)))/K);
    }
    /// ans.fs = average number of banana monkey can get.
    /// ans.sc = maximum number of banana monkey can get.
    return ans;
}
...
//    fwrite("out.txt");
    ...
    while(T--)
    {
        ...
        kmp(tword);
        pii ans = sol(0,0);
        double ok = (double)ans.sc - ans.fs;
        cout<<fixed<<"Case #"<<t++<<": "<<setprecision(10)<<ok<<endl;
    }
    return 0;
}
