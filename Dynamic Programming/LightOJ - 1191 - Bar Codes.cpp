...
ll sol(int pos,int remaining_bar,int m)
{
    if(pos<0 or remaining_bar<0)return 0;///invalid
    if(pos==0)return remaining_bar==0;///if no remaining bar then 1 solution else none
    ll &ans = dp[pos][remaining_bar][m];
    if(~ans)return ans;
    ans = 0;
    For(i,1,m)
    {
        /// making i length bar from this position
        ans+=sol(pos-i,remaining_bar-1,m);
    }
    return ans;
}
