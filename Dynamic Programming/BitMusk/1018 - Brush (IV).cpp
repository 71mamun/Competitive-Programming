...
{
    return ((b.sc-c.sc)*a.fs+(c.sc-a.sc)*b.fs+(a.sc-b.sc)*c.fs?false:true);
}

int solve(int mask)
{
    if(count_one(mask)==0) return 0;
    if(count_one(mask)==1)return 1;
    int &ans = dp[mask];
    if(~ans) return ans;
    ans = inf;
    For(i,0,n-1)
    {
        if(is_on(mask,i))
        {
            For(j,i+1,n-1)
            {
                if(is_on(mask,j))
                {
                    ans = min(ans,solve(mask&(~line[i][j]))+1);
                }
            }
            break;
        }
    ...
    while(T--)
    {
        ...
        For(i,0,n-1)
        {
            For(j,i+1,n-1)
            {
                For(k,0,n-1)
                {
                    if(is_collinear(point[i],point[j],point[k]))
                    {
                        line[i][j] = line[j][i] = sett(line[i][j],k);
                    }
                }
            }
        }
