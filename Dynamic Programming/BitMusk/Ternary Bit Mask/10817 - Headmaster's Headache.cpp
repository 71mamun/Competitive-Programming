...
#define LM 110
#define SLM 15
int shift[SLM],S,N,M,salary[LM],highest,dp[LM][20000];
vector<int>teach[LM];

void initialize_ternary_mask()
{
    shift[0]=1;
    For(i,1,11)shift[i]=3*shift[i-1];
}
...
{
    if(idx>=N)return (mask==highest?0:inf);
    if(mask==highest)return 0;
    int &ret = dp[idx][mask];
    if(~ret) return ret;
    ret = solve(idx+1,mask);
    int lim = sz(teach[idx])-1;
    For(i,0,lim)
    {
        int sub = teach[idx][i];
        int complete = getbit(mask,sub);
        if(complete<2)
        {
            mask = setbit(mask,sub,complete+1);
        }
    }
    ret = min(ret,solve(idx+1,mask)+salary[idx]);
    return ret;
}

int main()
{
    int mask,cost,tot_cost,sub;
    string line;
    stringstream ss;
    initialize_ternary_mask();
    while(tk(S,M,N) and S+M+N)
    {
        highest = shift[S]-1;
        getline(cin,line);
        mask = 0;tot_cost = 0;
        For(i,0,M-1)
        {
            ...
            while(ss>>sub)
            {
                int complete = getbit(mask,sub-1);
                if(complete<2)
                {
                    mask = setbit(mask,sub-1,complete+1);
                }
            }
        }
        For(i,0,N-1)
