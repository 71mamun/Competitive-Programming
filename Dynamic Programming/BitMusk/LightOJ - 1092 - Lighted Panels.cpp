...
#define LM 10
int row_mask[LM],R,C,t=1;
int dp[LM][257][257];

int sol(int crow,int pmask,int cmask);
...
int gen(int crow,int ccol,int pmask,int cmask,int nmask)
{
    if(ccol==C)
    {
        if(pmask==0)return sol(crow+1,cmask,nmask);
        else return inf;
    }
    int ans = inf;
    ans = min(ans,gen(crow,ccol+1,pmask,cmask,nmask));
    cmask = flip(cmask,ccol);
    nmask = flip(nmask,ccol);
    if(crow>0)
    {
        pmask = flip(pmask,ccol);
        if(ccol>0)pmask = flip(pmask,ccol-1);
        if(ccol<C-1)pmask = flip(pmask,ccol+1);
    }
    if(ccol>0)
    {
        cmask = flip(cmask,ccol-1);
        nmask = flip(nmask,ccol-1);
    ...
    ans = min(ans,gen(crow,ccol+1,pmask,cmask,nmask)+1);
    return ans;
}
...
int sol(int crow,int pmask,int cmask)
{
    if(crow==R)return (pmask==0?0:inf);
    int &ans = dp[crow][pmask][cmask];
    if(~ans)return ans;
    return ans = gen(crow,0,pmask,cmask,row_mask[crow+1]);
}
...
int main()
{
    ...
    while(T--)
    {
        ...
        For(i,0,R-1)
        {
            ...
            {
                sf(" %c",&temp);
                if(temp=='.')row_mask[i] = sett(row_mask[i],j);
            }
        }
        pf("Case %d: ",t++);
        int ans = sol(0,0,row_mask[0]);
        if(ans>1e9)pf("impossible\n");
        else pf("%d\n",ans);
    }
    return 0;
}
