...
ll C,R;
ll sol(ll r,ll cmask);
ll dp[30][19690],shift[12];
...
{
    rFor(i,5,0)cout<<getbit(mask,i);
    put("");
}

ll gen(ll r,ll c,ll cmask,ll nmask)
{
    if(c==C)return sol(r+1,nmask);
    if(int b1=getbit(cmask,c))return gen(r,c+1,cmask,setbit(nmask,c,b1==2));
    ll ret = gen(r,c+1,setbit(cmask,c,1),setbit(nmask,c,2));
    if(c<C-2&&!getbit(cmask,c+1)&&!getbit(cmask,c+2))
    {
        ret+=gen(r,c+3,cmask,nmask);
    }
    return ret;
}
