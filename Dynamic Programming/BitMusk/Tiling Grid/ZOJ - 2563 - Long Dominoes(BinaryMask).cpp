...
ll C,R;
ll sol(ll r,ll cmask,ll nmask);
ll dp[30][512][512];

ll gen(ll r,ll c,ll cmask,ll nmask,ll nnmask)
{
    if(c==C)return sol(r+1,nmask,nnmask);
    if(is_on(cmask,c))return gen(r,c+1,cmask,nmask,nnmask);
    ll ret = gen(r,c+1,sett(cmask,c),sett(nmask,c),sett(nnmask,c));
    if(c<C-2&&!is_on(cmask,c+1)&&!is_on(cmask,c+2))
    {
        ret+=gen(r,c+3,cmask|(1<<c)|(1<<(c+1))|(1<<(c+2)),nmask,nnmask);
    }
    return ret;
}
...
ll sol(ll r,ll cmask,ll nmask)
{
    ...
    ll &ans = dp[r][cmask][nmask];
    if(~ans)return ans;
    return ans = gen(r,0,cmask,nmask,0);
}

int main()
{
    while(sf("%lld %lld",&C,&R)==2 and C+R)
    {
        mem(dp,-1);
        pf("%lld\n",sol(0,0,0));
    }
