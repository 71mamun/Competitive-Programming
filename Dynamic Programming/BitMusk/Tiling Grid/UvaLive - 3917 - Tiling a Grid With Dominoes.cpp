...
ull generate(ull r,ull c,ull current_mask,ull next_musk);

ll sol(ull r,ull current_mask)
{
    if(r==h)return current_mask==0;
    ull &ans = dp[r][current_mask];
    if(vis[r][current_mask]==t)return ans;
    vis[r][current_mask]=t;
    return ans = generate(r,0,current_mask,0);
}

ull generate(ull r,ull c,ull current_mask,ull next_mask)
{
    if(c==4)
    {
