...
ull sol(int r,int cmask);
void print_mask(int mask);

ull gen(int r,int c,int cmask,int nmask)
{
    if(c==C)
    {
        return sol(r+1,nmask);
    }
    if(is_set(cmask,c))return gen(r,c+1,cmask,nmask);
    ull res = 0;
    int tmp;
    if(!is_set(nmask,c))
    {
        ///*
        ///*
        res+=gen(r,c+1,cmask,sett(nmask,c));
        if(c<C-1 and !is_set(nmask,c+1))
        {
            ///*
            ///**
            tmp = sett(nmask,c);
            tmp = sett(tmp,c+1);
            res+=gen(r,c+1,cmask,tmp);
        }
        if(c>0 and !is_set(nmask,c-1))
        {
            /// *
            ///**
            tmp = sett(nmask,c);
    ...
    if(c<C-1 and !is_set(cmask,c+1))
    {
        ///**
        res+=gen(r,c+2,cmask,nmask);
        if(!is_set(nmask,c))
        {
            ///**
            ///*
            tmp = sett(nmask,c);
            res+=gen(r,c+2,cmask,tmp);
        }
        if(!is_set(nmask,c+1))
        {
            ///**
...
ull sol(int r,int cmask)
{
    if(r==R)return cmask==0;
    ull &ans = dp[r][cmask];
    if(vis[r][cmask]==t)return ans;
    vis[r][cmask]=t;
    return ans = gen(r,0,cmask,row_mask[r+1]);
}
