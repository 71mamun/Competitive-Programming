...
int generate(int r,int c,int current_mask,int next_musk);

int sol(int r,int current_mask)
{
    if(r==h)return current_mask==0;
    int &ans = dp[r][current_mask];
    if(vis[r][current_mask]==t)return ans;
    vis[r][current_mask]=t;
    return ans = generate(r,0,current_mask,0);
}

int generate(int r,int c,int current_mask,int next_mask)
{
    if(c==4)
    {
        return sol(r+1,next_mask);
    }
    int ans = 0;
    if(is_on(current_mask,c))return generate(r,c+1,current_mask,next_mask);
    ans = generate(r,c+1,sett(current_mask,c),sett(next_mask,c));
    if(c<3 and !is_on(current_mask,c+1))
    {
        current_mask = sett(current_mask,c);
        current_mask = sett(current_mask,c+1);
        ans += generate(r,c+2,current_mask,next_mask);
    }
    return ans;
}
