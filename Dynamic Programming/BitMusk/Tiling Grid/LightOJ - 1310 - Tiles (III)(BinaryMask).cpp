...
int gen(int r,int c,int cmask,int nmask,int nnmask)
{
    ...
    int res = gen(r,c+1,cmask,nmask,nnmask);
    if(c<C-1 and !is_set(cmask,c+1) and !is_set(nmask,c) and !is_set(nmask,c+1) and !is_set(nnmask,c) and !is_set(nnmask,c+1))
    {
        res = max(res,gen(r,c+2,cmask,nmask|(1<<c)|(1<<(c+1)),nnmask|(1<<c)|(1<<(c+1)))+1);
    }
    if(c<C-2 and !is_set(cmask,c+1) and !is_set(cmask,c+2) and !is_set(nmask,c)and !is_set(nmask,c+1) and !is_set(nmask,c+2))
    {
        res = max(res,gen(r,c+3,cmask,nmask|(1<<c)|(1<<(c+1)|(1<<(c+2))),nnmask)+1);
    }
    return res;
}
...
int sol(int r,int cmask,int nmask)
{
    if(r==R)
    {
        return cmask==0?0:-inf;
    }
    int &ans = dp[r][cmask][nmask];
    if(~ans)return ans;
    ans = -inf;
    return ans = max(ans,gen(r,0,cmask,nmask,row_mask[r+2]));
}
