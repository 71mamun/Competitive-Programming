...
ull dp[105][5024];
bool vis[105][5024];
ull sol(ull r,ull current_mask);

ull gen(ull r,ull c,ull current_mask,ull next_mask)
{
    if(c==C)return sol(r+1,next_mask);
    ull ans = 0;
    if(is_on(current_mask,c))return gen(r,c+1,current_mask,next_mask);
    ans = gen(r,c+1,sett(current_mask,c),sett(next_mask,c));
    if(c<C-1 && !is_on(current_mask,c+1))
    {
        current_mask = sett(current_mask,c);
        current_mask = sett(current_mask,c+1);
        ans += gen(r,c+2,current_mask,next_mask);
    }
    return ans;
}
...
ull sol(ull r,ull current_mask)
{
    if(r==R)return current_mask == 0;
    ull &ans = dp[r][current_mask];
    bool &flg = vis[r][current_mask];
    if(flg)return ans;
    flg = true;
    return ans = gen(r,0,current_mask,0);
}
