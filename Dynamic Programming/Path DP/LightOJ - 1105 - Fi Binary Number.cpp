...
ll no_of_sol[50][3],nth;int highest;
ll sol(int len,int pre)
{
    if(len<=0)return 1;
    ll &ans = no_of_sol[len][pre];
    if(~ans)return ans;
    ans = 0;
    if(~pre)
    {
...
void print(int len,int pre,int th)
{
    if(len<=0)return;
    ll nsol = 0;
    ll prensol = 0;
...
ll sol(int len,int pre)
{
    if(len<=0)return pre!=-1;
    ll &ans = no_of_sol[len][pre];
    if(~ans)return ans;
    ans = 0;
    if(pre==-1)ans+=sol(len-1,-1);
//    if(ans>1e9)return ans;
    if(pre!=-1)ans+=sol(len-1,0);
//    if(ans>1e9)return ans;
    if(pre!=1)ans+=sol(len-1,1);
//    if(ans>1e9)return ans;
    return ans;
...
void path(int len,int pre,int th)
{
    if(len<=0)return;
    ll ans = 0, preans = 0;
    if(pre==-1)
