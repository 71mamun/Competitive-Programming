...
int sol(int les,int grt,int pos)
{
    if(pos<0)return 0;
    int &ans = dp[les][grt][pos];
    if(~ans)return ans;
    ans = -1;
    int lb = (les?0:is_on(l,pos));
    int rb = (grt?1:is_on(r,pos));
    For(i,lb,rb)
    {
        ans = max(ans,sol(les||i!=lb,grt||i!=rb,pos-1)+(i==1));
    }
    return ans;
}

void path(int les,int grt,int pos)
{
    if(pos<0)return;
    int ans = sol(les,grt,pos);
    int lb = (les?0:is_on(l,pos));
    int rb = (grt?1:is_on(r,pos));
    For(i,lb,rb)
    {
        int tmp = sol(les||i!=lb,grt||i!=rb,pos-1)+(i==1);
        if(tmp==ans)
        {
            if(i==0)res = setf(res,pos);
            else res = sett(res,pos);
            path(les||i!=lb,grt||i!=rb,pos-1);
            return;
        }
    }
    return;
