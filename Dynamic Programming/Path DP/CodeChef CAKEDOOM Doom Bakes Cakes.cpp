...
string line,res;int l,k,dp[110][12][12];

int sol(int pos,int frst,int prv)
{
    if(pos==l)return 1;
    int &ret = dp[pos][frst][prv];
    if(~ret)return ret;
    ret = 0;
    if(line[pos]!='?')
    {
        int now = line[pos]-'0';
        if(now==prv or (pos==l-1 and frst==now))return ret = 0;
        ret |= sol(pos+1,frst==k?now:frst,now);
    }
    else
    {
        For(i,0,k-1)
        {
            if(i==prv or (pos==l-1 and frst==i))continue;
            ret |= sol(pos+1,frst==k?i:frst,i);
        }
    }
    return ret;
}

void path(int pos,int frst,int prv)
{
    if(pos==l)return;
    int ret = sol(pos,frst,prv);
    if(line[pos]!='?')
    {
        int now = line[pos]-'0';
        res+=line[pos];
        path(pos+1,frst==k?now:frst,now);
        return;
    }
    else
    {
        ...
        {
            if(i==prv or (pos==l-1 and frst==i))continue;
            int tmp = sol(pos+1,frst==k?i:frst,i);
            if(tmp==ret)
            {
                res+=char(i+'0');
                path(pos+1,frst==k?i:frst,i);
                return;
            }
        }
    }
