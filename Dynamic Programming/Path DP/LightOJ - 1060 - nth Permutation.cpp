...
ll sol(int mask)
{
    ...
    {
        /**
            if I find that the previous char I didn't use which is same as this char
            then I used it in this position before which generate the #of solution using same
            char. but if i already used it then i can use this char now.
        */
        if(pre!=-1 and line[pre]==line[i] and !is_set(mask,pre))continue;
        if(!is_set(mask,i))
        {
            ans+=sol(sett(mask,i));
            /**
                i don't have to find solution greater then 2^31.
            */
...
void path(int mask,int nth)
{
    if(count_one(mask)==l)return;
    ll pre=0,ans=0;
    int upre = -1;
    For(i,0,l-1)
    {
        if(upre!=-1 and line[upre]==line[i] and !is_set(mask,upre))continue;
        if(!is_set(mask,i))
        {
            ans += sol(sett(mask,i));
            if(ans>=nth)
