...
pii solve(int st, int en,int now)
{
    if(st>en)return mp(0,0);
    pii &ret = dp[st][en][now];
    if(ok[st][en][now])return ret;
    ok[st][en][now]=true;
    pii ans,tmp;
    ans.fs = ans.sc = tmp.fs = tmp.sc = -inf;
    if(now==1)
    {
        For(i,st,en)
        {
            tmp = solve(i+1,en,2);
            int cal = arr[i]-arr[st-1]+tmp.fs;
            if(ans.fs<cal)
            {
                ans = mp(cal,tmp.sc);
            }
