...
vector<int>G[LM];
int dp[LM][4],N;
int solve(int u,int p,bool guarded)
{
    int l = G[u].size();
    int &ans = dp[u][guarded];
    if(~ans)return ans;
    ans = 0;
    For(i,0,l-1)
    {
        int v = G[u][i];
        if(v==p)continue;
        if(guarded)ans+=min(solve(v,u,false),solve(v,u,true));
        else ans+=solve(v,u,true);
    }
    return ans = (guarded?ans+1:ans);
}
int main()
{
    int u,v,l;
    while(tk(N) and N)
    {
        ...
        put((N==1?1:min(solve(1,-1,true),solve(1,-1,false))));
    }
    return 0;
}
