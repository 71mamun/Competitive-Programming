...
ll solve(ll u,ll p,ll k);

ll choose(ll u,ll p,ll k,ll i)
{
    ll l = sz(adj[u]);
    if(i>=l)return (k?inf:0);
    ll &ret = cdp[u][k][i];
    if(~ret)return ret;
    ret = inf;
    ll v = adj[u][i];
    if(p==v)return choose(u,p,k,i+1);
    For(j,0,k)
    {
        ret = min(ret,solve(v,u,j)+choose(u,p,k-j,i+1)+(j==0));
    }
    return ret;
}
...
ll solve(ll u,ll p,ll k)
{
    ...
    if(k==P)
    {
        For(i,0,l-1)
        {
            ...
            if(p==v)continue;
            /**
                +(p==-1) condition er jagay +1 korsilam tai protek call e ekta
                kore extra edge delete kortesi add hoitesilo ans er sathe. asole
                parent er sathe ektai edge delete kore ei node theke p ta niye
                calculate hobe. ei jonne ekbare +1 hobe first call e 1 add korechi.
            */
            ret = min(ret,solve(v,u,k) + (p==-1));
        }
    }
    ret = min(ret,choose(u,p,k-1,0));
    return ret;
}
