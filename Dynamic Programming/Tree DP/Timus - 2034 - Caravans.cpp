...
/**
There are several settlements connected by two-way 1 length roads.
The caravan goes from settlement s to f using one of the shortest routes.
Settlement r is the villain’s den. A band of robbers from settlement r has
received an assignment to rob the caravan. In the evening they will have
an exact plan of the route that the caravan will take the next morning.
During the night they will be able to move to any settlement on the route,
even to settlement  s or  f. They will lay an ambush there and rob the
caravan in the morning. Of course, among all such settlements the robbers
will choose the one closest to settlement  r.The robbers have a lot of time
until the evening. They don’t know the caravan’s route yet, but they want to
know the maximum distance they will have to go in the worst case to the
settlement where they will rob the caravan. calculate this distance.
*/
...
G = original graph
dag = dag graph
sdis = dis of all node from start
*/
vector<int>G[LM],dag[LM];
int sdis[LM],fdis[LM],rdis[LM],vis[LM],dp[LM],n,m;
...
void make_dag(int p,int u,int start,int finish)
{
    if(~vis[u])return;
    vis[u]=0;
    For(i,0,sz(G[u])-1)
    {
        int v = G[u][i];
        if(v==p)continue;
        if(sdis[finish]==sdis[v]+fdis[v] and fdis[u]>fdis[v] and sdis[u]<sdis[v])
        {
            dag[u].pub(v);
            make_dag(u,v,start,finish);
        }
    }
}
...
int sol(int u)
{
    ...
    return ans = (tmp>-1?min(ans,tmp):ans);
}
