...
int solve(int v,int c,int now)
{
    ...
    if(is_vowel(s[now]))
    {
        return ret = solve(v+1,0,now+1);
    }
    else if(s[now]!='?')
    {
        return ret = solve(0,c+1,now+1);
    }
    else
    {
        int a = solve(v+1,0,now+1);
        int b = solve(0,c+1,now+1);
        if(a == 0 or b==0)return ret = 0;
        int c = a+b;
        return ret = (c>0?1:(c<0?-1:0));
    }
}
