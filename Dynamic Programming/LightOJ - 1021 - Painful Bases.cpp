...
template<typename T> inline T setf(T &mask,int pos){mask=((mask)&(~(1LL<<pos)));}
template<typename T> inline T sett(T &mask,int pos){mask=((mask)|(1LL<<pos));}
template<typename T> inline T flip(T &mask,int pos){mask=((mask)^(1LL<<pos));}
///~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
ll solve(ll mask,ll mod)
{
    ...
    For(i,0,15)
    {
        if(!isOn(mask,i) and num[i])
        {
            ans+=solve((mask|(1LL<<i)),(mod+((i*bigMod(base,used,k))%k))%k);
        }
    }
    return ret = ans;
}
