...
double L,n,C,lo,hi,mid,org_lp,tmp_lp,angle;
...
int main()
{
    ...
    while(T--)
    {
        ...
        double ans = 0;
        org_lp = (1+n*C)*L;
        lo = L/2,hi = 1e5;/// L highest 2*R hote pare,
        if(abs(org_lp-L)>eps)
        {
            while(true)
            {
                ...
                angle = rAng(mid,mid,L);
                tmp_lp = angle*mid;
                if(tmp_lp>org_lp)lo = mid;
                else hi = mid;
            }
            ans = mid-sqrt(sqr(mid)-sqr(L/2));
        }
        pf("Case %d: %.10lf\n",t++,ans);
    }
    return 0;
