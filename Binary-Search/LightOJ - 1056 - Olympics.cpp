...
double calculate(double height)
{
    ...
    double redius = pdis(mp(0.0,0.0),mp(width/2,height/2));
    double angle = rAng(redius,redius,height);
    return 2*(width+(angle*redius));
}


int main()
{
    int T,t=1;double a,b,lo,hi,mid,perimeter;
    sf("%d",&T);
    while(T--)
    {
        sf(" %lf : %lf",&a,&b);
        ratio = a/b,lo = 0, hi = 400;
        while(true)
        {
            mid = ((lo+hi)/2);
            perimeter = calculate(mid);
            if(abs(perimeter-400.0)<eps)break;
            if(perimeter>400.0)
            {
                hi = mid;
            }
