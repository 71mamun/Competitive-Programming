...
ll to_int(string s){ll r=0;istringstream sin(s);sin>>r;return r;}
///~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#define MAX (1000000000010LL)
vector<ll>coin,lucky_numbers;
ll tot_coin,tot_lucky_numbers;

inline void calculate(int st,ll now)
{
    For(i,st,tot_coin-1)
    {
        if(now<=MAX/coin[i])
        {
            calculate(i,now*coin[i]);
            lucky_numbers.pub(now*coin[i]);
        }
        else break;
    }
}
...
int main()
{
    ...
    lucky_numbers.erase(unique(lucky_numbers.begin(),lucky_numbers.end()),lucky_numbers.end());
    tot_lucky_numbers = lucky_numbers.size();
    ll T,t=1,lo,hi;
    sf("%lld",&T);
    while(T--)
    {
        sf("%lld %lld",&lo,&hi);
        pf("Case %lld: %d\n",t++,upper_bound(all(lucky_numbers),hi)-lower_bound(all(lucky_numbers),lo));
    }
    return 0;
}
