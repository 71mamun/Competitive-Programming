...
double x,y,c,lo,hi,mid,cal_c,xp,yp;
...
{
    if(width>=x or width>=y)return 0;
    xp = sqrt(sqr(x)-sqr(width));
    yp = sqrt(sqr(y)-sqr(width));
    pdd p = calculate_intersection(create_equation(mp(0,0),mp(width,yp)),create_equation(mp(width,0),mp(0,xp)));
    if(p.fs<eps and p.sc<eps)return 0;
    return p.sc;
}
