...
int main()
{
    int T,n,mn,mx,t=1,*lb,*ub,loc_ans;
    sf("%d",&T);
    while(T--)
    {
        ll ans = 0;
        ...
        For(i,0,n-1)
        {
            For(j,i+1,n-1)
            {
                mx = arr[i]+arr[j];
                mn = abs(arr[i]-arr[j]);
                ub = upper_bound(arr,arr+n,mx-1);
                lb = lower_bound(arr,arr+n,mn+1);
                loc_ans = ub-lb;
                if(arr[i]>mn and arr[i]<mx)loc_ans--;
                if(arr[j]>mn and arr[j]<mx)loc_ans--;
                ans+=loc_ans;
            }
        }
        pf("Case %d: %lld\n",t++,ans/3);
