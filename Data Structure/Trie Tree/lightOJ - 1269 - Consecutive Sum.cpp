...
///~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
template<typename T> inline bool isOn(T &mask,ll pos){return((mask)&(1LL<<pos));}
template<typename T> inline T setf(T mask,ll pos){return mask=((mask)&(~(1LL<<pos)));}
template<typename T> inline T sett(T mask,ll pos){return mask=((mask)|(1LL<<pos));}
template<typename T> inline T flip(T mask,ll pos){return mask=((mask)^(1LL<<pos));}
///~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
struct node
{
    ...
    node()
    {
        for(ll i = 0; i<charsetsize; i++)next[i] = 0;
        endMark = 0;
        marked = 0;
    }
}data[SZ];
...
void print_dic(ll curr)
{
    if(data[curr].endMark!=0)
    {
        putv(prnt);
    }
    for(ll i=0; i<charsetsize; i++)
    {
...
int main()
{
    ...
    while(T--)
    {
        init();
        sf("%lld",&n);
        ll csum = 0,ansmx = 0,ansmn=(1LL<<54);
        Insert(csum);
        For(i,0,n-1)
        {
            sf("%lld",arr+i);
            csum^=arr[i];
            ll findmx = create_max(csum);
            ll findmn = csum;
            ll smx = Search(findmx);
            ll smn = Search(findmn);
            ansmx = max(ansmx,smx^csum);
            ansmn = min(ansmn,smn^csum);
            Insert(csum);
        }
        pf("Case %lld: %lld %lld\n",t++,ansmx,ansmn);
        Delete();
