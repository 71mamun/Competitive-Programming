...
int main()
{
    ...
    for(ll i=0;i<sz(str);++i)
    {
        ...
        if(idx.find(w)==idx.end())idx[w] = idx_mark++;

        ll ru = find_rip(u);
        ll rv = find_rip(v);

        sol[idx[w]] += (mem[ru]*mem[rv]);
    ...
    {
        cin>>L>>R;
        auto l = lower_bound(wstr.begin(),wstr.end(),L);
        auto r = upper_bound(wstr.begin(),wstr.end(),R);
        cout<<sol[idx[*(r-1)]]-sol[idx[*l]-1]<<endl;
    }
