...
#define LM 150010
int T,N,Q,t=1,arr[LM],tree[4*LM],qry_res,ans[4*LM];

/**
    "ans" store which one is attacking next.
    "tree" store how many are alive till.
*/
...
int query(int node,int b,int e,int k)
{
    if(b==e)
    {
        tree[node]=0;/// disable the current man
        return ans[node];
    }
    int left = (node<<1);
    int right = left+1;
    int mid = ((b+e)>>1);
    int ans;
    /// choosing which side to go.
    if(tree[left]>=k)ans = query(left,b,mid,k);
    else ans = query(right,mid+1,e,k-tree[left]);
    tree[node] = tree[left]+tree[right];
    return ans;
}
...
void update(int node,int b,int e,int idx,int v)
{
    ...
    int mid = ((b+e)>>1);
    /// choosing which side to go.
    if(idx<=mid)update(left,b,mid,idx,v);
    else update(right,mid+1,e,idx,v);
    tree[node] = tree[left]+tree[right];
}
...
//    fread("in.txt");
    ...
    while(T--)
    {
        ...
        For(i,0,Q-1)
        {
            ...
            if(opr=='c')
            {
                qry_res = query(1,1,N+Q,v);
                if(qry_res==-1)pf("none\n");
                else pf("%d\n",qry_res);
            }
            else
            {
                update(1,1,N+Q,++uped,v);
            }
        }
    }
    return 0;
