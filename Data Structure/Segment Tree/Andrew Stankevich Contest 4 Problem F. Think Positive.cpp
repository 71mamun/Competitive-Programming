...
void init(int node,int sb,int se)
{
    if(sb==se)
    {
        tree[node] = arr[sb];
...
int qry(int node,int sb,int se,int qs,int qe)
{
    if(qe<sb||se<qs)return inf;
    if(qs<=sb&&se<=qe)return tree[node];

    int mid = (sb+se)/2;
    int left = node*2;
    int right = left+1;

    int lft = qry(left,sb,mid,qs,qe);
    int rgt = qry(right,mid+1,se,qs,qe);
    return min(lft,rgt);
}
