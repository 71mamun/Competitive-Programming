...
int query(int node,int b,int e,int i,int j)
{
    ...
    int right = left+1;
    int mid = ((b+e)>>1);
    int left_ans = query(left,b,mid,i,j);
    int right_ans = query(right,mid+1,e,i,j);
    return (arr[left_ans]<=arr[right_ans]?left_ans:right_ans);
}
/**
    the 'c' is min bar of range [a,b]
    then the optimal ans is:
        max(sol(a,c-1),sol(c+1,b),[a,b])
*/
int calculate(int b,int e)
{
    if(b>e)return 0;
    if(b==e)return arr[b];
    int part = query(1,1,n,b,e);
    int left = calculate(b,part-1);
    int right = calculate(part+1,e);
    int ans = arr[part]*(e-b+1);
