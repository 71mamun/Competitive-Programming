...
void update(int node,int b,int e,int i,int v)
{
    ...
    if(b>=i and e<=i)
    {
        tree[node] = (tree[node]%mod+v%mod)%mod;
        return;
    }
    int left = (node<<1);
    int right = left+1;
...
int main()
{
    ...
    while(T--)
    {
        ...
        For(i,1,N)arr[i]=Map[arr[i]];
        ll ans = 0;
        For(i,1,N)
        {
            /// how many IS are there which max element is less than arr[i].
            int qry_res = query(1,1,N,1,arr[i]-1)%mod;
            /// adding ans;
            ans = (ans%mod+qry_res%mod+1)%mod;
            /// there are total "qry_res+1" IS which max element is at most arr[i].
            update(1,1,N,arr[i],qry_res+1);
        }
        pf("Case %d: %d\n",t++,ans);
    }
    return 0;
