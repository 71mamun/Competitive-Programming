...
ll query(ll node,ll b,ll e,ll i,ll j,ll carry)
{
    ...
    if(b>=i and e<=j)return tree[node]+carry*(e-b+1);
    ll left = (node<<1);
    ll right = left+1;
    ll mid = ((e+b)>>1);
    ll la = query(left,b,mid,i,j,carry+prop[node]);
    ll ra = query(right,mid+1,e,i,j,carry+prop[node]);
    return la+ra;
}

int main()
{
    ll N,Q,T,opr,i,j,t=1;ll v;
    sf("%lld",&T);
    while(T--)
    {
        sf("%lld %lld",&N,&Q);
        For(k,0,4*N)tree[k]=prop[k]=0;
        pf("Case %lld:\n",t++);
        while(Q--)
        {
            sf("%lld",&opr);
