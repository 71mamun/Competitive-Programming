...
void update(int node,int b,int e,int i,int j)
{
    ...
    {
        tree[node]+=1;/// Update it
        if(b!=e)/// not a leaf node
        {
            lazy[left]+=1;/// mark child as lazy
            lazy[right]+=1;/// mark child as lazy
            upreq[left]=upreq[right]=true;
        }
        return;
    }
    update(left,b,mid,i,j);/// Updating left child
    update(right,mid+1,e,i,j); /// Updating right child
}
...
int query(int node,int b,int e,int p)
{
    ...
    if(b>e or p<b or p>e)return 0;/// Current segment is not within range [i, j]
    if(b==p and e==p)/// Current segment is totally within range [i, j]
    {
        return tree[node];
    }
    if(p<=mid)return query(left,b,mid,p);
    else return query(right,mid+1,e,p);
}
...
//    fread("in.txt");
    ...
    while(T--)
    {
        ...
        sort(all(point));
        int idx=1;
        /// maping the points to decrease the distinct point.
        /// to making the segment tree possible
        For(i,0,sz(point)-1)
        {
            if(!Map[point[i]])Map[point[i]] = idx++;
        }
        For(k,0,4*idx)tree[k]=lazy[k]=upreq[k] = false;
        For(i,0,N-1)
        {
            update(1,1,idx-1,Map[SEG[i].fs],Map[SEG[i].sc]);
        }
        printf("Case %d:\n",t++);
        For(i,0,Q-1)
        {
            printf("%d\n",query(1,1,idx-1,Map[QRY[i]]));
        }
    }
    return 0;
}
