...
int query(int node,int sb,int se,int qb,int qe)
{
    if(qb>se or qe<sb)return 0;
    if(qb<=sb and qe>=se)return tree[node];
    int left = (node<<1);
    int right = left+1;
    int mid = ((sb+se)>>1);
    int left_ans = query(left,sb,mid,qb,qe);
    int right_ans = query(right,mid+1,se,qb,qe);
...
int main()
{
    ...
    while(T--)
    {
        ...
        {
            sf("%d %d",&l,&h);
            int left_seg = (l==1?0:query(1,1,N,1,l-1));
            int right_seg = (h==N?0:query(1,1,N,h+1,N));
            pf("%d\n",__gcd(left_seg,right_seg));
        }
    }
    return 0;
}
