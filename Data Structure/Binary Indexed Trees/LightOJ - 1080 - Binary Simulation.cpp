...
//input
template<class T1> void tk(T1 &e1){cin>>e1;}
template<class T1,class T2> void tk(T1 &e1,T2 &e2){cin>>e1>>e2;}
template<class T1,class T2,class T3> void tk(T1 &e1,T2 &e2,T3 &e3){cin>>e1>>e2>>e3;}
template<class T1,class T2,class T3,class T4> void tk(T1 &e1,T2 &e2,T3 &e3,T4 &e4){cin>>e1>>e2>>e3>>e4;}
...
I main()
{
    ...
    for(I t=1; t<=T; t++)
    {
        ...
        while(m--)
        {
            ...
            {
                sf("%d %d",&x,&y);
                update(x,1);// update that each segment with x idx once.
                update(y+1,-1);// reduce the update which take place over those segment who has idx y+1.
            }
            else
            {
                sf("%d",&x);
                I times = query(x);
                if(times%2)pf("%c\n",(bin[x-1]=='1'?'0':'1'));//bin is 0 based
                else pf("%c\n",bin[x-1]);
            }
        }
    }
