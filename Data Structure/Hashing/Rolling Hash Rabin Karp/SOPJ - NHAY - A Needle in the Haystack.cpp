...
ll base1 = 29,rmod1 = 100019;
ll base2 = 1201,rmod2 = 1000000007;
...
ll Hash(string s,ll base,ll rmod)
{
    ll hv = 0;
    For(i,0,sz(s)-1)
    {
        hv = usmod(hv*base+s[i],rmod);
    }
    return hv;
}
...
//    fread("in.txt");
    ...
    {
        tk(needle);
        ll v1 = Hash(needle,base1,rmod1);
        ll v2 = Hash(needle,base2,rmod2);
        tk(haystack);
        ll hv1 = 0;
        ll hv2 = 0;
        ll reduce1 = bigMod(base1,l-1,rmod1);
        ll reduce2 = bigMod(base2,l-1,rmod2);
        For(i,0,sz(haystack)-1)
        {
            if(i<l)
            {
                hv1 = usmod(hv1*base1+haystack[i],rmod1);
                hv2 = usmod(hv2*base2+haystack[i],rmod2);
            }
            else
            {
                hv1 = usmod(hv1-haystack[i-l]*reduce1,rmod1);
                hv1 = usmod(hv1*base1+haystack[i],rmod1);
                hv2 = usmod(hv2-haystack[i-l]*reduce2,rmod2);
                hv2 = usmod(hv2*base2+haystack[i],rmod2);
                if(hv1==v1&&hv2==v2)cout<<i-l+1<<endl;
            }
        }
        cout<<endl;
    }
