...
Write a program to transform an array A[1], A[2],..., A[n] according to m instructions.
Each instruction (L, R, v, p) means: First, calculate how many numbers from A[L] to A[R]
(inclusive) are strictly less than v, call this answer k. Then, change the value of A[p]
to u*k/(R - L + 1), here we use integer division (i.e. ignoring fractional part).
*/
...
int query(int l,int r,ll h)
{
    ...
    For(i,lseg+1,rseg-1)
    {
        ans+=(upper_bound(all(seg[i]),h-1)-seg[i].begin());
    }
    For(i,l,(lseg+1)*rn-1)
    {
        if(arr[i]<h)ans++;
...
{
    int seg_id = idx/rn;
    int sze = sz(seg[seg_id]);
    int beg = rn*seg_id;
    arr[idx]=v;
    clr(seg[seg_id]);
    while(sze--)
    {
        seg[seg_id].pub(arr[beg]);
        beg++;
    }
    sort(all(seg[seg_id]));
}
...
int main()
{
    ...
    while(sf("%d %d %lld",&n,&m,&u)==3)
    {
        ...
        For(i,0,m-1)
        {
            sf("%d %d %lld %d",&l,&r,&v,&p);
            int k = query(l-1,r-1,v);
            update(p-1,(k*u)/(r-l+1));
        }
        For(i,0,n-1)
        {
            pf("%lld\n",arr[i]);
