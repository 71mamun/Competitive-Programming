...
int sliding_window(int v[], int n, int k)
{
    deque<pii>window;
    int ans = 0;
    For(i,0,n-1)
    {
        ...
        while(window.front().sc<=i-k)
            window.pop_front();
        if(i>=k-1 and window.front().fs-(i-k>=0?v[i-k]:0)<0)
        {
            ans++;
        }
    }
...
int main()
{
    ...
    {
        For(i,0,n-1)tk(v[i]);
        For(i,n,2*n-1)v[i]=v[i-n];
        For(i,1,2*n-1)v[i]+=v[i-1];
        put(n-sliding_window(v,2*n-1,n));
    }
    return 0;
}
