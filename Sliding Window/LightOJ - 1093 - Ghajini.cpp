...
void sliding_window_min(int n, int k)
{
    ...
    For(i,0,n-1)
    {
        while(!window.empty() and window.back().fs>=v[i])
            window.pop_back();
        window.push_back(make_pair(v[i],i));
        while(window.front().sc<=i-k)
            window.pop_front();
        if(i>=k-1)
        {
            Min[idx++] = window.front().fs;
        }
    }
}

void sliding_window_max(int n, int k)
{
    window.clear();
    int idx=0;
    For(i,0,n-1)
...
//    fwrite("WAOUT.out");
    ...
    {
        sf("%d %d",&n,&d);
        For(i,0,n-1)sf("%d",v+i);
        sliding_window_min(n,d);
        sliding_window_max(n,d);
        int ans = -1;
        For(i,0,n-d)
        {
