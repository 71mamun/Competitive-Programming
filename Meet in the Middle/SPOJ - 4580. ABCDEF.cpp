...
#define LM 110
#define GLM 1000005
ll N,arr[LM],fgrp[GLM],sgrp[GLM],fidx,sidx,intersection[3*GLM];

int main()
{
    while(scanf("%lld",&N)==1)
    {
        For(i,0,N-1)scanf("%lld",arr+i);fidx=0,sidx=0;
        For(i,0,N-1)
        {
            For(j,0,N-1)
            {
                For(k,0,N-1)
                {
                    fgrp[fidx++] = arr[i]*arr[j]+arr[k];
                    if(arr[i]!=0)sgrp[sidx++] = arr[i]*(arr[j]+arr[k]);
                }
            }
        }
        ...
        ll i=0,j=0,ffrq=1,sfrq=1,ans=0;

        while(i<fidx and j<sidx)
        {
            while(i<fidx-1 and fgrp[i] == fgrp[i+1])i++,ffrq++;
            while(j<sidx-1 and sgrp[j] == sgrp[j+1])j++,sfrq++;
            if(fgrp[i]==sgrp[j])
            {
                ans+=(ffrq*sfrq);
                i++,j++,ffrq=sfrq=1;
            }
            else if(fgrp[i]>sgrp[j])
            {
                j++,sfrq=1;
            }
            else
