...
#define LM 40
#define GLM 131100
ll T,t=1,N,W,arr[LM],fgrp[GLM],sgrp[GLM],A,B;
...
ll calculate()
{
    ...
    idx=mask=0;
    while(mask<(1<<(N-divider)))
    {
        ll sum = 0;
        For(i,divider,N-1)
        {
            if(is_on(mask,i-divider))sum+=arr[i];
        }
        sgrp[idx++]=sum;
        mask++;
    }
    ...
    For(i,0,slm-1)
    {
        ans+=upper_bound(fgrp,fgrp+flm,B-sgrp[i])-lower_bound(fgrp,fgrp+flm,A-sgrp[i]);
    }
    return ans;
}
