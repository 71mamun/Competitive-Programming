...
#define LM 20
#define GLM 32770
ll T,t=1,N,W,arr[LM],fgrp[GLM],sgrp[GLM],shift[LM];
bool ok;

void initialize_ternary_mask()
{
    shift[0]=1;
    For(i,1,18)shift[i]=3*shift[i-1];
}
...
ll calculate()
{
    ...
    ll divider = N/2;
    ll idx = 0;
    ll flm = shift[divider];
    ll slm = shift[N-divider];
    while(mask<flm)
    {
        ll sum = 0;
        For(i,0,divider)
    ...
    while(mask<slm)
    {
        ...
        For(i,divider,N-1)
        {
            sum+=(getbit(mask,i-divider)*arr[i]);
        }
        sgrp[idx++]=sum;
        mask++;
    }
    ...
    For(i,0,slm-1)
    {
        if(binary_search(fgrp,fgrp+flm,W-sgrp[i]))return true;
    }
    return false;
}
