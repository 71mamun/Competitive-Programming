...
#define     lsOne(S)        ((S)&(-(S)))
#define     repi(i,a,b)     for(I i=a; i<=b; ++i)
...
struct tpl
{
    ...
    bool operator < (const tpl& p) const
    {
        return w<p.w;
    }
};

I n,m,ac,u,v,w,par[SZ];
vector<tpl>e;

I findSet(I r)
{
    return (par[r]==r?r:par[r]=findSet(par[r]));
}
...
I main()
{
    ...
    while(T--)
    {
        ...
        sort(all(e));
        I aair=n,air=n,pcost = air*ac;
        repi(i,0,m-1)
        {
            u = e[i].u,v=e[i].v,w=e[i].w;
            I pu = findSet(u),pv = findSet(v);
            if(pu!=pv)
            {
                par[pu] = pv;
                air--;
