...
#define LM 5100
char grid[LM][LM];
pair<int,int>grid_info[LM][LM];
int P,W,N,row,column;
vector<int>adj[LM];
...
void row_calculate()
{
    ...
    For(i,1,N)
    {
        For(j,1,N)
        {
            if(grid[i][j]!='X')
            {
                grid_info[i][j].fs=row;
            }
            else
            {
                while(grid[i][j]=='X' and j<=N)j++;
                j--;
                row++;
            }
        }
...
void column_calculate()
{
    ...
    For(i,1,N)
    {
        For(j,1,N)
        {
            if(grid[j][i]!='X')
            {
                grid_info[j][i].sc=column;
            }
            else
            {
                while(grid[j][i]=='X' and j<=N)j++;
                j--;
                column++;
            }
        }
    ...
    while(scanf("%d",&N)==1)
    {
        ...
        For(i,1,N)
        {
            For(j,1,N)
            {
                if(grid[i][j]!='#')
                {
                    adj[grid_info[i][j].first].pub(grid_info[i][j].second);
                }
            }
        }
        int ans = max_bpm(row);
        pf("%d\n",ans);
        For(i,0,row)adj[i].clear();
    }
    return 0;
}
