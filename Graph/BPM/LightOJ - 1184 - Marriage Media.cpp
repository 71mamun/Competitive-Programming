...
bool check(tpl m,tpl w)
{
    if(abs(m.h-w.h)<=12 and abs(m.a-w.a)<=5 and m.d==w.d)return true;
    return false;
}
...
I dfs(I u)
{
    ...
    col[u]=true;
    for(I i=0;i<G[u].size();++i)
    {
        I v = G[u][i];
        if(arr[v]==-1 or dfs(arr[v]))
        {
            arr[v]=u;
            return true;
        }
...
I main()
{
    ...
    while(T--)
    {
        ...
        for(I i=0;i<m;++i)
        {
            for(I j=0;j<n;++j)
            {
                if(check(b[i],g[j]))G[i].pub(j);
            }
        }
        pf("Case %d: %d\n",++t,match());
        b.clear();g.clear();
        for(I i=0;i<m;++i)G[i].clear();
    }
    return 0;
}
