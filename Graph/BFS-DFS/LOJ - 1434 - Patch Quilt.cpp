...
{
    int x,y;
    if(pos>=sz(name))return 1;
    int ans = 0;
    for(int i=0;i<8;++i)
    {
        x = r+X[i];
        y = c+Y[i];
        if(x<0||x>=r||y<0||y>=c||g[x][y]!=name[pos])continue;
        ans|=dfs(x,y,pos+1);
    }
    return ans;
}
...
{
    int cur = ti(name[0]);
    for(int i=0;i<sz(loc[cur]);++i)
    {
        pii pos = loc[cur][i];
        if(dfs(pos.fs,pos.sc,1))
        {
            pf("%s found: (%d,%d)",name.c_str(),pos.fs,pos.sc);
...
int main()
{
    ...
    while(T--)
    {
        for(int i=0;i<LM;++i)loc[i].clear();
        cin>>r>>c;
        for(int i=0;i<r;++i)
        {
            for(int j=0;j<c;++j)
