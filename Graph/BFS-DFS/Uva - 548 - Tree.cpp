...
int sol(int pb,int pe,int ib,int ie)
{
    if(pb>pe||ib>ie)return 1e9;
    if(pb==pe||ib==ie)
    {
        child[post[pb]] = 1;
        return post[pb];
    }
    int ipos = in_pos[post[pe]];
    int s1 = ipos-ib;
    int s2 = ie-ipos;
    int lft = sol(pb,pb+(s1-1),ib,ipos-1);
    int rgt = sol(pe-s2,pe-1,ipos+1,ie);
    if(lft<1e5)adj[post[pe]].pb(lft);
    if(rgt<1e5)adj[post[pe]].pb(rgt);
    return post[pe];
}
