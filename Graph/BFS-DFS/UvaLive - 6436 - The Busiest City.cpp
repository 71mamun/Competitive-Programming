...
int N;
vector<int>graph[LM];
int node[LM];/// stores #node in subtree rooted as a node with out parents node.
int par[LM];/// stores the parent of a node.
/// 1 is always taken as the root of the main tree. in 129 line dfs call.
...
int main()
{
    ...
    while(T--)
    {
        ...
        For(u,1,N)
        {
            ll loc_mx=0,tot_child=0;
            For(i,0,sz(graph[u])-1)
            {
                int v = graph[u][i];
                if(par[u]!=v)
                {
                    loc_mx+=(node[v]*(N-node[v]-1));
                    tot_child+=node[v];/// #node in the subtree rooted as v with out its #parent
                }
            }
            int parent = N-tot_child-1;/// #parent of node u.
            loc_mx += parent*tot_child;
            mx = max(mx,(loc_mx/2));
        }
        pf("Case #%d: %d\n",t++,mx);
    }
