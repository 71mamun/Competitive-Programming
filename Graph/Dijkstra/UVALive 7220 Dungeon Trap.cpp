...
int grid[LM][LM],scost[LM][LM],dcost[LM][LM];
int n,m;
int T,t=1,sr,sc,dr,dc,tot;

void djk(int sr,int sc,int dist[LM][LM])
{
    ...
    while(!pq.empty())
    {
        ...
        For(i,0,3)
        {
            ...
            if(dist[nr][nc]>dist[top.r][top.c]+grid[nr][nc])
            {
                dist[nr][nc] = dist[top.r][top.c]+grid[nr][nc];
    ...
    while(T--)
    {
        ...
        if(scost[dr][dc]<inf)
        {
            ...
            For(i,0,n-1)
            {
                For(j,0,m-1)
                {
                    if((i==dr&&j==dc)||(i==sr&&j==sc)||scost[i][j]>=inf||dcost[i][j]>=inf)continue;
                    ans = max(ans,tot-scost[i][j]-dcost[i][j]+2*grid[i][j]);
                }
            }
        }
