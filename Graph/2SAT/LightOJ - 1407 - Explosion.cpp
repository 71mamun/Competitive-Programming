...
/**
invite "1" is node no 2 and don't invite "1" is node no 3. the compliment of a node is (node_no^1).
how to make a node strictly true? just add a edge from it's compliment to it. node no start is 2 to n*2+1
*/

const int SZ = 2*1000+50;
...
tpl cnlr[7];
I low[SZ],trv[SZ],c[SZ],n,d,cmp,k,gi[SZ];
VI take;
VI G[SZ];
SI stk;
...
bool twoSat(I n)
{
    ...
    for(I i=2;i<=n+1;i+=2)
    {
        if(gi[i]==gi[i^1])return false;
        else if(gi[i]<gi[i^1])take.pub(i/2);
    }
    return true;
}
...
/**
back track for finding solution with councilor opinion.
*/

bool solve(I idx)
{
    if(idx>=k)return twoSat((n<<1));
    tpl &p = cnlr[idx];
    G[p.u^1].pub(p.u);
...
//    fread("in.txt");
    ...
    while(T--)
    {
        ...
        for(I i=0;i<k;i++)
        {
            sf("%d %d %d %d",&typ,&cnlr[i].u,&cnlr[i].v,&cnlr[i].w);
            cnlr[i].u<<=1,cnlr[i].v<<=1,cnlr[i].w<<=1;
            if(typ==2)cnlr[i].u^=1,cnlr[i].v^=1,cnlr[i].w^=1;
        }
        if(solve(0))
        {
            pf("Case %d: ",++t);
        ...
        else pf("Case %d: ",++t),pf("Impossible.\n");
