...
const int SZ = 10010;
vec<I>G[SZ];
I c[SZ],l[SZ],t[SZ],d,col[SZ],minAns,art,cunt;
ll minWay,n;
bool artp[SZ];
set<I>ok;

void articulationPoint(I u,I p,I r)
{
    c[u]=0;
    l[u]=t[u]=++d;
    I m = G[u].size(),cnt=0;
    for(I i=0; i<m; i++)
    {
        ...
        {
            cnt++;
            articulationPoint(v,u,r);
            l[u]=min(l[u],l[v]);
            if(l[v]>=t[u]&&u!=r)artp[u] = true,cunt++;
        }
        else
        {
            l[u]=min(l[u],t[v]);
    ...
    if(cnt>1&&u==r)artp[u]=true,cunt++;
}

I shaftsCount(I u,I p)
{
    c[u]=1;
    I m = G[u].size(),node=0;
    for(I i=0; i<m; i++)
    {
        I v = G[u][i];
        if(v==p)continue;
        if(artp[v]&&ok.find(v)==ok.end())
        {
            ok.insert(v);
            art++;
        }
        else if(c[v]==-1&&artp[v]==false)
        {
            node+=shaftsCount(v,u);
        }
    }
    return node+1;
}
...
I main()
{
    ...
    {
        sf("%llu %d",&n,&m);
        for(I i=0; i<=n; i++)G[i].clear();
        mems(artp,false);
        mems(c,-1);
        d=-1,minAns=0,minWay=1;
        cunt=0;
        I M=m;
        while(M--)
        {
        ...
        for(I i=0; i<n; i++)
        {
            ...
            if(c[i]==-1&&!artp[i])
            {
                I loc = shaftsCount(i,-1);
                if(art==1)minWay *= loc,minAns++,flag = false;;
            }
        }
        if(flag)pf("Case %d: %d %llu\n",++t,2,(n*(n-1))/2);
        else pf("Case %d: %d %llu\n",++t,minAns,minWay);
    }
    return 0;
}
