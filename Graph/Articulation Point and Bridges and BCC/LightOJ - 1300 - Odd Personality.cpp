...
vec<I>tG[SZ];
set<pii>b;
I n,c[SZ],l[SZ],t[SZ],d,rip[SZ],mem[SZ],col[SZ];
...
void dfs(I u,I p,I r)
{
    ...
    for(I i=0; i<m; i++)
    {
        ...
        if(c[v]==-1)
        {
            ...
            dfs(v,u,r);
            l[u]=min(l[u],l[v]);
            if(l[v]>t[u])b.insert(mp(min(u,v),max(u,v)));
        }
        else
        {
            l[u]=min(l[u],t[v]);
...
void group_dfs(I u,I p,I r)
{
    ...
    for(I i=0; i<m; i++)
    {
        I v = G[u][i];
        if(v==p)continue;
        if(c[v]!=-1||b.find(mp(min(u,v),max(u,v)))!=b.end())continue;
        group_dfs(v,u,r);
        make_frnd(v,r);
    }
}
...
bool bicol_chk(I u,I pc)
{
    ...
    for(I i=0;i<m;i++)
    {
        ...
        if(c[v]==-1)
        {
            if(bicol_chk(v,c[u]))return true;
        }
        else if(c[v]==c[u]) return true;
    }
    return false;
}
...
I main()
{
    ...
    {
        sf("%d %d",&n,&e);
        for(I i=0;i<n;i++)tG[i].clear(),G[i].clear();
        while(e--)
        {
            sf("%d %d",&u,&v);
            G[u].pub(v);
        ...
        for(I i=0;i<n;i++)
        {
            if(c[i]==-1&&sz(tG[i])>0&&col[rip[i]]==-1)
            {
                if(bicol_chk(i,1))
                {
                    ans+=mem[i];
