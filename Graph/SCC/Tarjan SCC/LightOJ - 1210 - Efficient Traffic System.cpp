...
{
    componentNo = Time = 0;
    memset(color,-1,sizeof(int)*(noOfNode+1));// because of 1 base 'i' is set -1 from 0 to noOfNode.
    while(!stk.empty())stk.pop();
    for(I i=1;i<=noOfNode;i++)// because of 1 base 'i' is started form 1.
    {
...
int main()
{
    int totalTestCase,testCaseNo=1;
    sf("%d",&totalTestCase);
    while(totalTestCase--)
    {
        sf("%d %d",&noOfNode,&noOfEdge);
        while(noOfEdge--)
        {
            sf("%d %d",&from,&to);
            graph[from].pub(to);
        }
        int totalComponent = tarjanSCC();
        memset(inDeg,0,sizeof(int)*totalComponent);
        memset(outDeg,0,sizeof(int)*totalComponent);
        createDag();
        zeroIndeg=zeroOutDeg=0;
        for(int i=0;i<totalComponent;i++)
        {
            if(inDeg[i]==0)zeroIndeg++;
            if(outDeg[i]==0)zeroOutDeg++;
        }
        pf("Case %d: %d\n",testCaseNo++,(totalComponent==1?0:max(zeroIndeg,zeroOutDeg)));
        for(int i=0;i<=noOfNode;i++)graph[i].clear(),convertedGrpah[i].clear();
    }
    return 0;
}
