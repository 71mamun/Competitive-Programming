...
#define SZ 10004
I n=1005,m,u,v,low[SZ],trv[SZ],c[SZ],d,cmp,gi[SZ];
VI G[SZ],tG[SZ];
SI stk;
...
I main()
{
    ...
    while(T--)
    {
        ...
        memset(c,0,sizeof(c));
        ans = supDfs(gi[0]);
        for(int i=com;i>=0;i--)if(c[i]==0){ans=false;break;}
        pf("Case %d: %s\n",++t,(ans?"YES":"NO"));
        for(I i=0;i<=n;i++)clr(G[i]),clr(tG[i]);
    }
    return 0;
}
