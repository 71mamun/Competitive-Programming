...
void kmp(char v[],int pi)
{
    lvrus[pi]= strlen(v);
    lps[pi][0]=0;
    for(int i=1;i<lvrus[pi];++i)
    {
        int j = lps[pi][i-1];
        while(j>0&&v[i]!=v[j])
            j = lps[pi][j-1];
...
int kmp_dp(int len,int now,int pi)
{
    int &ret = kmpdp[len][now][pi];
    if(~ret)return ret;
    while(len>0 && virus[pi][len]!=now)
        len = lps[pi][len-1];
    if(virus[pi][len]==now) len++;
    return ret = len;
}
...
int main()
{
	...
	while(T--)
	{
        ...
        For(i,0,l-1)
        {
            For(j,0,n-1)
            {
                matched[j] = kmp_dp(matched[j],line[i],j);
                if(matched[j]>=lvrus[j])
                {
                    last[j] = i-lvrus[j]+1;
                }
            }
