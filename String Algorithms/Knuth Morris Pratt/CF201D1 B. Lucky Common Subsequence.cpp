...
int kmp_dp(int len,int now)
{
    int &ret = kmpdp[len][now];
    if(~ret)return ret;
    while(len>0&&virus[len]!=now)
        len = lps[len-1];
    if(virus[len]==now) len++;
    return ret = len;
}

int sol(int i,int j,int matched)
{
    if(i>=sz(a)||j>=sz(b))return 0;
    int &ret = dp[i][j][matched];
    if(~ret)return ret;
    ret = max(sol(i+1,j,matched),sol(i,j+1,matched));
    if(a[i]==b[j])
    {
        /// khali b leki vule
        if(kmp_dp(matched,a[i])<sz(virus))
            /// matched+1 diye call kortesilam
            ret = max(ret,sol(i+1,j+1,kmp_dp(matched,a[i]))+1);
    }
    return ret;
}

void path(int i,int j,int matched)
{
    if(i>=sz(a)||j>=sz(b))return;
    int ret = sol(i,j,matched);
    if(ret == sol(i+1,j,matched))return path(i+1,j,matched);
    if(ret == sol(i,j+1,matched))return path(i,j+1,matched);
    if(a[i]==b[j])
    {
        /// matched+1 diye call kortesilam
        /// virus er jaygay b liksilam
        if(kmp_dp(matched,a[i])<sz(virus)&&ret == sol(i+1,j+1,kmp_dp(matched,a[i]))+1)
        {
            cout<<a[i];
            /// matched+1 diye call kortesilam
            return path(i+1,j+1,kmp_dp(matched,a[i]));
        }
    }
}
