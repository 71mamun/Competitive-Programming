...
int sol(int len,int pos)
{
    if(pos>=line.size())return 0;
    int &ans = dp[len][pos];
    if(~ans)return ans;
    ans = inf;
    int tlen = kmp_dp(len,line[pos]);
    if(tlen<sz(virus))ans = min(ans,sol(tlen,pos+1));
    ans = min(ans,sol(len,pos+1)+1);
    return ans;
}

void path(int len,int pos)
{
    if(pos>=line.size())
    {
        cout<<endl;
    ...
    int tlen = kmp_dp(len,line[pos]);
    int ans = sol(len,pos);
    if(tlen<sz(virus)&&ans == sol(tlen,pos+1))
    {
        cout<<line[pos];
        path(tlen,pos+1);
    }
