...
dd ppdis(pdd a,pdd b){return sqrt((dd)sqr(a.fs-b.fs)+sqr(a.sc-b.sc));}
...
dd ternarySearch(pdd a,pdd b,pdd c,pdd d)
{
    while(true)
    {
        pdd flm = mp((2*a.fs+b.fs)/3,(2*a.sc+b.sc)/3);
        pdd frm = mp((a.fs+2*b.fs)/3,(a.sc+2*b.sc)/3);
        pdd slm = mp((2*c.fs+d.fs)/3,(2*c.sc+d.sc)/3);
        pdd srm = mp((c.fs+2*d.fs)/3,(c.sc+2*d.sc)/3);
        dd disl = ppdis(flm,slm);
        dd disr = ppdis(frm,srm);
        if(abs(disl-disr)<eps)return (disl+disr)/2;
        if(disl<disr) b = frm, d = srm;
        else a = flm, c = slm;
    }
}
...
I main()
{
    ...
    while(T--)
    {
        tk(a.fs,a.sc,b.fs,b.sc);tk(c.fs,c.sc,d.fs,d.sc);
        pf("Case %d: %.7lf\n",++t,ternarySearch(a,b,c,d));
    }
    return 0;
}
