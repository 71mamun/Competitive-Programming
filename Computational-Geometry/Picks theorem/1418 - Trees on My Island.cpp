...
ll ComputeSignedArea(const vector<pii> &p)
{
    ll area = 0;
    for(int i = 0; i < p.size(); i++)
    {
        int j = (i+1) % p.size();
        area += (p[i].fs*p[j].sc - p[j].fs*p[i].sc);
    }
    return abs(area / 2);
}

ll ComputelatticePointsOnBoundary(const vector<pii> &p)
{
    ll B = 0;
    for(int i = 0; i < p.size(); i++)
    {
        int j = (i+1) % p.size();
        B += __gcd(abs(p[j].fs-p[i].fs),abs(p[j].sc-p[i].sc))+1;
    }
    return B-p.size();
}
...
int main()
{
    ...
    while(T--)
    {
        ...
        ll area = ComputeSignedArea(points);
        ll B = ComputelatticePointsOnBoundary(points);
