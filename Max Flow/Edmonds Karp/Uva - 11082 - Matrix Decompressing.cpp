...
int main()
{
    ...
    while(T--)
    {
        ...
        For(i,1,R)
        {
            ek.add_edge(0,i,rc[i]-rc[i-1]-C);
        }
        For(i,1+R,C+R)
        {
            ek.add_edge(i,R+C+1,cc[i-R]-cc[i-1-R]-R);/// don't try to increase the back flow it may cancel by forward flow.
        }
        For(i,1,R)
        {
            For(j,1+R,C+R)
        ...
        int mxflow = ek.max_flow(0,R+C+1);
        cout<<"Matrix "<<t++<<endl;
        For(j,1+R,C+R)
        {
            For(i,1,R)
            {
                mat[i][j-R] = ek.residual[j][i];
            }
        }
