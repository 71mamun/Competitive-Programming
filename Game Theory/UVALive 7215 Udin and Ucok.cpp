...
int grd(int x,int y)
{
    ...
    ans = 0;
    set<int>g;
    g.insert(grd(0,x)^grd(x-2,x));
    g.insert(grd(0,x)^grd(x-3,x));
    for(int i=1;i<x-2;++i)
    {
        g.insert(grd(i,x)^grd(x-3-i,x));
    }
    g.insert(grd(x-2,x)^grd(0,x));
    g.insert(grd(x-3,x)^grd(0,x));
    while(g.find(ans)!=g.end())
        ans++;
    return ans;
}
