...
int chk(int r1,int c1,int r2,int c2)
{
    int &ans = cdp[r1][c1][r2][c2];
    if(~ans)return ans;
    ans = 0;
    for(int i=r1;i<=r2;++i)
    {
        for(int j=c1;j<=c2;++j)
        {
            if(mat[i][j]!=2&&mat[i][j]!=3&&mat[i][j]!=5&&mat[i][j]!=7)
                return ans = 0;
        }
    }
    return ans = 1;
...
int grundyNumber(int r1,int c1,int r2,int c2)
{
    if(r2==r1&&c2==c1)return 0;
    if(chk(r1,c1,r2,c2))return 0;
    ...
    for(int i=r1; i<r2; ++i)
    {
        int g1 = grundyNumber(r1,c1,i,c2);
        int g2 = grundyNumber(i+1,c1,r2,c2);
        col[g1^g2] = 1;
    }

    for(int i=c1; i<c2; ++i)
    {
        int g1 = grundyNumber(r1,c1,r2,i);
        int g2 = grundyNumber(r1,i+1,r2,c2);
        col[g1^g2] = 1;
    }
...
int main()
{
    ...
    while(T--)
    {
        ...
        if(grundyNumber(0,0,n-1,n-1))pf("First");
        else pf("Second");
        pf("\n");
    }
    return 0;
