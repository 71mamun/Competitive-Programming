...
/**
n will be wining position if their is a
loss position between [ceil(n/2),n-1]interval
because i can reduce the ball lowest to ceil(n/2).
else n is a losing position.
    1 is 1st loss position
    1*2+1=3 is the 2nd loss position
    3*2+1=7 is the 3rd loss position
    7*2+1=15 is the 4th loss position
if n is a loss position then the nest loss position is (n)*2+1.
and if m = (n)*2+1 then there are not any winning position
between [ceil(m/2) which is n+1,m-1] interval
*/
