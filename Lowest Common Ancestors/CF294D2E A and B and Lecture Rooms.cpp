...
#define MAXN 100100
#define step 18    /// step=log(n)
vector<int>adj[MAXN];
int node[MAXN],n,parent[MAXN][step+1],start[MAXN],finish[MAXN],T,level[MAXN];// T = time
...
{
    T=0;/// to calculate finish and start
    for(int i=0;i<=n;i++)
        adj[i].clear();
}
...
int main()
{
    ...
    while(q--)
    {
        ...
        int totdis = level[u]+level[v]-2*level[lca];
        if(totdis%2!=0)
        {
            pf("0\n");
            continue;
        ...
        totdis = totdis/2;
        int mid = kth_parent(u,totdis);
        int ans = 0;
        if(mid==lca)
        {
            ans = n;
            ans-=node[kth_parent(u,totdis-1)];
            ans-=node[kth_parent(v,totdis-1)];
        }
        else
        {
            ans = node[kth_parent(u,totdis)];
            ans-=node[kth_parent(u,totdis-1)];
        }
        pf("%d\n",ans);
    }
